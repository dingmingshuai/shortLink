...
public class LinkUtil {
    ...
    public static long getLinkCacheValidTime(Date validDate) {
        return Optional.ofNullable(validDate)
                .map(each -> DateUtil.between(new Date(), each, DateUnit.MS))//不为空，设置有效期为当前时间与有效期时间的差值
                .orElse(DEFAULT_CACHE_VALID_TIME);//validDate为null，则为永久有效，设置默认值为一个月
    }
