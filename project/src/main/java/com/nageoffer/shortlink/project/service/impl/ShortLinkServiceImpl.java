...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    @Transactional(rollbackFor = Exception.class)//创建短链接增加事务方法,生成短链接异常时，回滚
    @Override
    public ShortLinkCreateRespDTO createShortLink(ShortLinkCreateReqDTO requestParam) {
        verificationWhitelist(requestParam.getOriginUrl());//验证原始链接是否在白名单内
...
//            }
        ...
        if (!Objects.equals(hasShortLinkDO.getValidDateType(), requestParam.getValidDateType())
                || !Objects.equals(hasShortLinkDO.getValidDate(), requestParam.getValidDate())) {//修改后的短链接有效期不一致
            stringRedisTemplate.delete(String.format(GOTO_SHORT_LINK_KEY, requestParam.getFullShortUrl()));//则删除之前存储的旧的无效跳转缓存
            if (hasShortLinkDO.getValidDate() != null && hasShortLinkDO.getValidDate().before(new Date())) {//过期短链接修改后
                if (Objects.equals(requestParam.getValidDateType(), VailDateTypeEnum.PERMANENT.getType()) || requestParam.getValidDate().after(new Date())) {//如果改为永久有效或者增加有效期
                    stringRedisTemplate.delete(String.format(GOTO_IS_NULL_SHORT_LINK_KEY, requestParam.getFullShortUrl()));//删除之前存储的旧的无效跳转缓存
                }
            }
        }
    }
...
//        http://192.168.0.1:8080/app/somepage.jsp 访问时就是"192.168.0.1"
        ...
        if (StrUtil.isNotBlank(originalLink)){
            ...
            shortLinkStats(fullShortUrl, null, statsRecord);//统计短链接信息
        ...
        try {
            ...
            ShortLinkStatsRecordDTO statsRecord = buildLinkStatsRecordAndSetUser(fullShortUrl, request, response);
            shortLinkStats(fullShortUrl, shortLinkDO.getGid(), statsRecord);//统计短链接信息
    ...
    public void shortLinkStats(String fullShortUrl, String gid, ShortLinkStatsRecordDTO statsRecord) {
        ...
        shortLinkStatsSaveProducer.send(producerMap);//获取短链接发送到状态生产者队列
    }
