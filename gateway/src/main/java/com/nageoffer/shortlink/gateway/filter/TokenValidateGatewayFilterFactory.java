 ...
 *
 * @Author DubPAN
 * @Create2024/6/21 14:19
 * @Version 1.0
 */
@Component  // 创建完生成SpringBoot Bean
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            String requestPath = request.getPath().toString();//请求路径
            String requestMethod = request.getMethod().name();//get  /  post
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {//验证当前请求路径是否在白名单中
                ...
                ServerHttpResponse response = exchange.getResponse();//不在白名单中，返回错误信息
            ...
            return chain.filter(exchange);//在白名单中，直接进行下一层filter过滤
        };
    }

    private boolean isPathInWhiteList(String requestPath, String requestMethod, List<String> whitePathList) {//验证请求路径是否在白名单中
